package cn.edu.ncepu.crypto.algebra.algorithms;
...
public class LagrangePolynomial {
	...
	public LagrangePolynomial(Pairing pairing, int degree, Element zeroValue) {
		...
		for (int i = 1; i < coef.length; i++) {
			this.coef[i] = this.pairing.getZr().newRandomElement().getImmutable();
		}
	}
	...
	public static Element calCoef(Pairing pairing, int[] set, int index) {
		...
		for (int i = 0; i < set.length; i++) {
			...
			Element member = pairing.getZr().newZeroElement().sub(elementSet[i]).getImmutable();
			Element denominator = elementIndex.sub(elementSet[i]).getImmutable();
