...
public class RBACLLW15AccessCredentialMGenerator implements PairingKeyParameterGenerator {
	...
	public PairingKeySerParameter generateKey() {
		if (params instanceof RBACLLW15AccessCredentialMGenParameter) {
			...
			RBACLLW15PublicKeySerParameter publicKeyParameters = (RBACLLW15PublicKeySerParameter) parameters
					.getPublicKeyParameter();
			RBACLLW15MasterSecretKeySerParameter masterSecretKeyParameters = (RBACLLW15MasterSecretKeySerParameter) parameters
					.getMasterSecretKeyParameter();

			Pairing pairing = PairingFactory.getPairing(publicKeyParameters.getParameters());
			Element[] elementRoles = PairingUtils.MapStringArrayToGroup(pairing, parameters.getRoles(),
					PairingUtils.PairingGroupType.Zr);
			Element elementTime = PairingUtils.MapStringToGroup(pairing, parameters.getTime(),
					PairingUtils.PairingGroupType.Zr);
			if (!parameters.isIntermediateGeneration()) {
				// generate medical staff access credential without using intermediate
				// parameters
				...
				Element a0 = masterSecretKeyParameters.getG2Alpha().mul(intermediateParameters.get_G_3_r())
						.getImmutable();
			...
			RBACLLW15PublicKeySerParameter publicKeyParameters = (RBACLLW15PublicKeySerParameter) parameters
					.getPublicKeyParameter();
			RBACLLW15AccessCredentialMSerParameter secretKeyParameters = (RBACLLW15AccessCredentialMSerParameter) parameters
			...
			Element elementDelegateRole = PairingUtils
					.MapStringToGroup(pairing, parameters.getDelegateRole(), PairingUtils.PairingGroupType.Zr)
					.getImmutable();
			if (!parameters.isIntermediateGeneration()) {
				// generate medical staff access credential without using intermediate
				// parameters
				...
				a0 = a0.mul(publicKeyParameters.getU0().powZn(secretKeyParameters.getElementTime())).powZn(t)
						.mul(secretKeyParameters.getA0()).mul(secretKeyParameters.getBsAt(parameters.getIndex())
								.powZn(elementRoles[parameters.getIndex()]))
						.getImmutable();

				return new RBACLLW15AccessCredentialMSerParameter(publicKeyParameters.getParameters(), roles,
						elementRoles, secretKeyParameters.getTime(), secretKeyParameters.getElementTime(), a0, a1, a2,
						bv, bs);
			} else {
				// generate medical staff access credential using intermediate parameters
				RBACLLW15IntermediateSerParameter intermediateParameters = parameters.getIntermediateParameters();
				...
				for (int i = 0; i < bs.length; i++) {
					if (secretKeyParameters.getRoleAt(i) != null) {
						...
						bs[i] = secretKeyParameters.getBsAt(i).mul(intermediateParameters.get_U_s_r_at(i))
								.getImmutable();
					}
				}
				// Compute the rest of a0
