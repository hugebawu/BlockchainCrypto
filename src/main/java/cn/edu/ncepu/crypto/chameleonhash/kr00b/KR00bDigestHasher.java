...
public class KR00bDigestHasher implements ChameleonHasher {
	...
	public byte[][] computeHash() throws CryptoException, DataLengthException {
		...
		try {
			return new byte[][] { encodeChameleonHashResult(cHashResult[0]),
	...
	public byte[][] computeHash(byte[] cHashResult, byte[] auxiliaryParameters)
		...
		try {
			return new byte[][] { encodeChameleonHashResult(cHashPrime[0]),
