...
import cn.edu.ncepu.crypto.encryption.sepe.SelfExtractableBaseEngine;
...
public class SelfExtractableCPABEEngine extends Engine {
	...
	public SelfExtractableCPABEEngine(CPABEEngine engine, PBEParametersGenerator pbeParametersGenerator,
			BlockCipher blockCipher, Digest digest) {
		super(engine.getEngineName(), engine.getProveSecModel(), engine.getPayloadSecLevel(),
				engine.getPredicateSecLevel());
		this.engine = engine;
		this.selfExtractableBaseEngine = new SelfExtractableBaseEngine(engine, pbeParametersGenerator, blockCipher,
				digest);
	}
	...
	public PairingKeyEncapsulationSerPair encapsulation(PairingKeySerParameter publicKey,
			PairingCipherSerParameter intermediate, String accessPolicy, byte[] ek) throws PolicySyntaxException {
		if (!(this.engine instanceof OOCPABEEngine)) {
			throw new IllegalArgumentException("Engine does not support online/offline mechanism");
		}
	...
	public PairingKeyEncapsulationSerPair encapsulation(PairingKeySerParameter publicKey,
			PairingCipherSerParameter intermediate, int[][] accessPolicyIntArrays, String[] rhos, byte[] ek) {
		if (!(this.engine instanceof OOCPABEEngine)) {
			throw new IllegalArgumentException("Engine does not support online/offline mechanism");
		}
		OOCPABEEngine ooEngine = (OOCPABEEngine) this.engine;
		PairingKeyEncapsulationSerPair encapsulationPair = ooEngine.encapsulation(publicKey, intermediate,
				accessPolicyIntArrays, rhos);
		return this.selfExtractableBaseEngine.encapsulation(encapsulationPair, ek);
	}

	public PairingKeyEncapsulationSerPair encapsulation(PairingKeySerParameter publicKey, String accessPolicy,
			byte[] ek) throws PolicySyntaxException {
		int[][] accessPolicyIntArrays = ParserUtils.GenerateAccessPolicy(accessPolicy);
		String[] rhos = ParserUtils.GenerateRhos(accessPolicy);
	...
	public PairingKeyEncapsulationSerPair encapsulation(PairingKeySerParameter publicKey, int[][] accessPolicyIntArrays,
			String[] rhos, byte[] ek) {
		PairingKeyEncapsulationSerPair encapsulationPair = this.engine.encapsulation(publicKey, accessPolicyIntArrays,
				rhos);
		return this.selfExtractableBaseEngine.encapsulation(encapsulationPair, ek);
	}

	public byte[] decapsulation(PairingKeySerParameter publicKey, PairingKeySerParameter secretKey, String accessPolicy,
			PairingCipherSerParameter header) throws PolicySyntaxException, InvalidCipherTextException {
		int[][] accessPolicyIntArrays = ParserUtils.GenerateAccessPolicy(accessPolicy);
		String[] rhos = ParserUtils.GenerateRhos(accessPolicy);
	...
	public byte[] decapsulation(PairingKeySerParameter publicKey, PairingKeySerParameter secretKey,
		...
		if (!(header instanceof SEPEHeaderParameter)) {
			PairingUtils.NotVerifyCipherParameterInstance(selfExtractableBaseEngine.getEngineName(), header,
		...
		byte[] k_prime_temp = this.engine.decapsulation(publicKey, secretKey, accessPolicyIntArrays, rhos,
				headerParameter);
