...
public class CPABEHW14DecryptionGenerator extends CPABERW13DecryptionGenerator {
	public void init(CipherParameters params) {
		...
		CPABEHW14PublicKeySerParameter oriPublicKeyParameter = (CPABEHW14PublicKeySerParameter) oriParameter
				.getPublicKeyParameter();
		CPABEHW14HeaderSerParameter oriHeaderParameter = (CPABEHW14HeaderSerParameter) oriParameter
		...
		for (String attribute : oriC1s.keySet()) {
			Element newC1 = oriC1s.get(attribute).mul(oriPublicKeyParameter.getW().powZn(oriC4s.get(attribute)))
					.getImmutable();
			newC1s.put(attribute, newC1);
		}
		for (String attribute : oriC2s.keySet()) {
			Element newC2 = oriC2s.get(attribute).mul(oriPublicKeyParameter.getU().powZn(oriC5s.get(attribute)))
					.getImmutable();
		...
		if (oriHeaderParameter instanceof CPABEHW14CiphertextSerParameter) {
			...
			CPABEHW14CiphertextSerParameter newCiphertextParameter = new CPABEHW14CiphertextSerParameter(
					...
					oriCiphertextParameter.getC0(), newC1s, newC2s, oriCiphertextParameter.getC3s(), oriC4s, oriC5s);
			CPABEDecryptionGenerationParameter resultParameter = new CPABEDecryptionGenerationParameter(
					...
					oriParameter.getSecretKeyParameter(), oriParameter.getAccessPolicy(), oriParameter.getRhos(),
			...
			CPABEHW14HeaderSerParameter newHeaderParameter = new CPABEHW14HeaderSerParameter(
					oriHeaderParameter.getParameters(), oriHeaderParameter.getC0(), newC1s, newC2s,
					oriHeaderParameter.getC3s(), oriC4s, oriC5s);
			CPABEDecryptionGenerationParameter resultParameter = new CPABEDecryptionGenerationParameter(
					oriParameter.getAccessControlEngine(), oriParameter.getPublicKeyParameter(),
					oriParameter.getSecretKeyParameter(), oriParameter.getAccessPolicy(), oriParameter.getRhos(),
					newHeaderParameter);
	...
	public Element recoverMessage() throws InvalidCipherTextException {
		...
		CPABEHW14CiphertextSerParameter ciphertextParameter = (CPABEHW14CiphertextSerParameter) this.parameter
				.getCiphertextParameter();
