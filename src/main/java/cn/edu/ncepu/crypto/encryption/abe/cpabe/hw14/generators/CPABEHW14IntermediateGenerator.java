...
public class CPABEHW14IntermediateGenerator implements PairingEncryptionGenerator {
	...
	protected void computeEncapsulation() {
		...
		for (int i = 0; i < n; i++) {
			...
			this.C1s[i] = publicKeyParameter.getW().powZn(lambdas[i]).mul(publicKeyParameter.getV().powZn(ts[i]))
					.getImmutable();
			this.C2s[i] = publicKeyParameter.getU().powZn(xs[i]).mul(publicKeyParameter.getH()).powZn(ts[i].negate())
					.getImmutable();
	...
	public PairingCipherSerParameter generateCiphertext() {
		...
		return new CPABEHW14IntermediateSerParameter(publicKeyParameter.getParameters(), n, sessionKey, s, C0, lambdas,
				ts, xs, C1s, C2s, C3s);
	}
}