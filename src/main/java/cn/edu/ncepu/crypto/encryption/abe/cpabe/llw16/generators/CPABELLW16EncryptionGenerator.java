...
import cn.edu.ncepu.crypto.encryption.abe.cpabe.hw14.generators.CPABEHW14EncryptionGenerator;
import cn.edu.ncepu.crypto.encryption.abe.cpabe.hw14.serparams.CPABEHW14IntermediateSerParameter;
...
public class CPABELLW16EncryptionGenerator extends CPABEHW14EncryptionGenerator {
	...
	public void init(CipherParameters parameter) {
		...
		if (oriEncryptionParameter.isIntermediateGeneration()) {
			...
			AsymmetricKeySerPairGenerator chKeyPairGenerator = oriEncryptionParameter
					.getChameleonHashKeyPairGenerator();
			KeyGenerationParameters chKeyPairGenerationParameter = oriEncryptionParameter
					.getChameleonHashKeyPairGenerationParameter();
		...
		CPABEEncryptionGenerationParameter resultEncryptionParameter = new CPABEEncryptionGenerationParameter(
				oriEncryptionParameter.getAccessControlEngine(), oriEncryptionParameter.getPublicKeyParameter(),
				oriEncryptionParameter.getAccessPolicy(), mappedStringRhos, oriEncryptionParameter.getMessage());
		if (oriEncryptionParameter.isIntermediateGeneration()) {
			CPABEHW14IntermediateSerParameter intermediateHW14 = new CPABEHW14IntermediateSerParameter(
					this.intermediate.getParameters(), this.intermediate.getN(), this.intermediate.getSessionKey(),
					this.intermediate.getS(), this.intermediate.getC0(), this.intermediate.getLambdas(),
					this.intermediate.getTs(), this.intermediate.getXs(), this.intermediate.getC1s(),
					this.intermediate.getC2s(), this.intermediate.getC3s());
	...
	protected void computeEncapsulation() {
		...
		try {
			...
			if (this.parameter.isIntermediateGeneration()) {
				...
				this.C02 = publicKeyParameter.getU().powZn(V).mul(publicKeyParameter.getH()).powZn(t0.negate())
						.getImmutable();
			}
	...
	public PairingKeyEncapsulationSerPair generateEncryptionPair() {
		...
		return new PairingKeyEncapsulationSerPair(this.sessionKey.toBytes(),
				new CPABELLW16HeaderSerParameter(publicKeyParameter.getParameters(), chameleonHash, r,
						chameleonHashPublicKey, C01, C02, C03, C0, C1s, C2s, C3s, C4s, C5s));
	}

	public PairingCipherSerParameter generateCiphertext() {
		computeEncapsulation();
		Element C = this.sessionKey.mul(this.parameter.getMessage()).getImmutable();
		return new CPABELLW16CiphertextSerParameter(publicKeyParameter.getParameters(), chameleonHash, r,
				chameleonHashPublicKey, C01, C02, C03, C, C0, C1s, C2s, C3s, C4s, C5s);
	}
}
