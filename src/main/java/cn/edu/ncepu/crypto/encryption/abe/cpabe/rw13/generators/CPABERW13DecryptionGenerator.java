...
import cn.edu.ncepu.crypto.encryption.abe.cpabe.rw13.serparams.CPABERW13CiphertextSerParameter;
import cn.edu.ncepu.crypto.encryption.abe.cpabe.rw13.serparams.CPABERW13HeaderSerParameter;
...
public class CPABERW13DecryptionGenerator implements PairingDecryptionGenerator, PairingDecapsulationGenerator {
	...
	protected void computeDecapsulation() throws InvalidCipherTextException {
		CPABERW13PublicKeySerParameter publicKeyParameter = (CPABERW13PublicKeySerParameter) this.parameter
				.getPublicKeyParameter();
		CPABERW13SecretKeySerParameter secretKeyParameter = (CPABERW13SecretKeySerParameter) this.parameter
				.getSecretKeyParameter();
		CPABERW13HeaderSerParameter ciphertextParameter = (CPABERW13HeaderSerParameter) this.parameter
				.getCiphertextParameter();
		AccessControlEngine accessControlEngine = this.parameter.getAccessControlEngine();
		Pairing pairing = PairingFactory.getPairing(publicKeyParameter.getParameters());
		try {
			AccessControlParameter accessControlParameter = accessControlEngine
					.generateAccessControl(this.parameter.getAccessPolicy(), this.parameter.getRhos());
			Map<String, Element> omegaElementsMap = accessControlEngine.reconstructOmegas(pairing,
					secretKeyParameter.getAttributes(), accessControlParameter);
			...
			for (String attribute : omegaElementsMap.keySet()) {
				...
				A = A.mul(
						pairing.pairing(C1, K1).mul(pairing.pairing(C2, K2)).mul(pairing.pairing(C3, K3)).powZn(lambda))
						.getImmutable();
			}
	...
	public Element recoverMessage() throws InvalidCipherTextException {
		...
		CPABERW13CiphertextSerParameter ciphertextParameter = (CPABERW13CiphertextSerParameter) this.parameter
				.getCiphertextParameter();
