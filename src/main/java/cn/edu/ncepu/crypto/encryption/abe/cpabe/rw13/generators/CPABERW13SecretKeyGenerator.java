...
public class CPABERW13SecretKeyGenerator implements PairingKeyParameterGenerator {
	...
	public PairingKeySerParameter generateKey() {
		CPABERW13MasterSecretKeySerParameter masterSecretKeyParameter = (CPABERW13MasterSecretKeySerParameter) parameter
				.getMasterSecretKeyParameter();
		CPABERW13PublicKeySerParameter publicKeyParameter = (CPABERW13PublicKeySerParameter) parameter
		...
		Element K0 = publicKeyParameter.getG().powZn(masterSecretKeyParameter.getAlpha())
				.mul(publicKeyParameter.getW().powZn(r)).getImmutable();
		...
		for (String attribute : attributes) {
			...
			Element K3i = publicKeyParameter.getU().powZn(elementAttribute).mul(publicKeyParameter.getH()).powZn(ri)
					.getImmutable();
