package cn.edu.ncepu.crypto.encryption.abe.kpabe;
...
public class SelfExtractableKPABEEngine extends Engine {
	...
	public SelfExtractableKPABEEngine(KPABEEngine engine, PBEParametersGenerator pbeParametersGenerator,
	...
	public PairingKeySerParameter keyGen(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
			String accessPolicy) throws PolicySyntaxException {
	...
	public PairingKeyEncapsulationSerPair encapsulation(PairingKeySerParameter publicKey,
			PairingCipherSerParameter intermediate, String[] attributes, byte[] ek) {
		if (!(this.engine instanceof OOKPABEEngine)) {
			throw new IllegalArgumentException("Engine does not support online/offline mechanism");
		}
		OOKPABEEngine ooEngine = (OOKPABEEngine) this.engine;
