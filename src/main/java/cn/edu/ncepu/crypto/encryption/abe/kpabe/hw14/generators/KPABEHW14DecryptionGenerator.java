package cn.edu.ncepu.crypto.encryption.abe.kpabe.hw14.generators;
...
public class KPABEHW14DecryptionGenerator extends KPABERW13DecryptionGenerator {
	public void init(CipherParameters params) {
		...
		KPABEHW14PublicKeySerParameter oriPublicKeyParameter = (KPABEHW14PublicKeySerParameter) oriParameter
				.getPublicKeyParameter();
		KPABEHW14HeaderSerParameter oriHeaderParameter = (KPABEHW14HeaderSerParameter) oriParameter
		...
		for (String attribute : oriC2s.keySet()) {
			Element newC2 = oriC2s.get(attribute).mul(oriPublicKeyParameter.getU().powZn(oriC3s.get(attribute)))
					.getImmutable();
		...
		if (oriHeaderParameter instanceof KPABEHW14CiphertextSerParameter) {
			...
			KPABEHW14CiphertextSerParameter newCiphertextParameter = new KPABEHW14CiphertextSerParameter(
					oriCiphertextParameter.getParameters(), oriCiphertextParameter.getC(),
					oriCiphertextParameter.getC0(), oriCiphertextParameter.getC1s(), newC2s, oriC3s);
			KPABEDecryptionGenerationParameter resultParameter = new KPABEDecryptionGenerationParameter(
					...
					oriParameter.getSecretKeyParameter(), oriParameter.getAttributes(), newCiphertextParameter);
			super.init(resultParameter);
		} else {
			KPABEHW14HeaderSerParameter newHeaderParameter = new KPABEHW14HeaderSerParameter(
					oriHeaderParameter.getParameters(), oriHeaderParameter.getC0(), oriHeaderParameter.getC1s(), newC2s,
					oriC3s);
			KPABEDecryptionGenerationParameter resultParameter = new KPABEDecryptionGenerationParameter(
					oriParameter.getAccessControlEngine(), oriParameter.getPublicKeyParameter(),
					oriParameter.getSecretKeyParameter(), oriParameter.getAttributes(), newHeaderParameter);
			super.init(resultParameter);
		}
	}
	...
	public Element recoverMessage() throws InvalidCipherTextException {
		...
		KPABEHW14CiphertextSerParameter ciphertextParameter = (KPABEHW14CiphertextSerParameter) this.parameter
				.getCiphertextParameter();
