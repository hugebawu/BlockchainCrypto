package cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16;
...
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.generators.KPABELLW16DecryptionGenerator;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.generators.KPABELLW16EncryptionGenerator;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.generators.KPABELLW16IntermediateGenerator;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.generators.KPABELLW16KeyPairGenerator;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.generators.KPABELLW16SecretKeyGenerator;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.serparams.KPABELLW16CiphertextSerParameter;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.serparams.KPABELLW16HeaderSerParameter;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.serparams.KPABELLW16IntermediateSerParameter;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.serparams.KPABELLW16MasterSecretKeySerParameter;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.serparams.KPABELLW16PublicKeySerParameter;
import cn.edu.ncepu.crypto.encryption.abe.kpabe.llw16.serparams.KPABELLW16SecretKeySerParameter;
...
public class OOKPABELLW16Engine extends OOKPABEEngine {
	...
	public PairingKeySerParameter keyGen(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
		...
		if (!(publicKey instanceof KPABELLW16PublicKeySerParameter)) {
			PairingUtils.NotVerifyCipherParameterInstance(SCHEME_NAME, publicKey,
					KPABELLW16PublicKeySerParameter.class.getName());
		}
		if (!(masterKey instanceof KPABELLW16MasterSecretKeySerParameter)) {
			PairingUtils.NotVerifyCipherParameterInstance(SCHEME_NAME, masterKey,
					KPABELLW16MasterSecretKeySerParameter.class.getName());
		}
	...
	public PairingCipherSerParameter encryption(PairingKeySerParameter publicKey, String[] attributes,
		...
		KPABEEncryptionGenerationParameter encryptionGenerationParameter = new KPABEEncryptionGenerationParameter(
				publicKey, attributes, message);
	...
	public Element decryption(PairingKeySerParameter publicKey, PairingKeySerParameter secretKey, String[] attributes,
		...
		if (!(secretKey instanceof KPABELLW16SecretKeySerParameter)) {
			PairingUtils.NotVerifyCipherParameterInstance(SCHEME_NAME, secretKey,
					KPABELLW16SecretKeySerParameter.class.getName());
		}
		if (!(ciphertext instanceof KPABELLW16CiphertextSerParameter)) {
			PairingUtils.NotVerifyCipherParameterInstance(SCHEME_NAME, ciphertext,
					KPABELLW16CiphertextSerParameter.class.getName());
		}
		KPABELLW16DecryptionGenerator decryptionGenerator = new KPABELLW16DecryptionGenerator();
		KPABEDecryptionGenerationParameter decryptionGenerationParameter = new KPABEDecryptionGenerationParameter(
	...
	public byte[] decapsulation(PairingKeySerParameter publicKey, PairingKeySerParameter secretKey, String[] attributes,
		...
		if (!(header instanceof KPABELLW16HeaderSerParameter)) {
			PairingUtils.NotVerifyCipherParameterInstance(SCHEME_NAME, header,
					KPABELLW16HeaderSerParameter.class.getName());
		}
		KPABELLW16DecryptionGenerator decryptionGenerator = new KPABELLW16DecryptionGenerator();
		KPABEDecryptionGenerationParameter decryptionGenerationParameter = new KPABEDecryptionGenerationParameter(
	...
	public PairingCipherSerParameter offlineEncryption(PairingKeySerParameter publicKey, int n) {
		...
		KPABEIntermediateGenerationParameter intermediateGenerationParameter = new KPABEIntermediateGenerationParameter(
				publicKey, n);
	...
	public PairingCipherSerParameter encryption(PairingKeySerParameter publicKey,
			PairingCipherSerParameter intermediate, String[] attributes, Element message) {
		if (!(publicKey instanceof KPABELLW16PublicKeySerParameter)) {
			PairingUtils.NotVerifyCipherParameterInstance(SCHEME_NAME, publicKey,
					KPABELLW16PublicKeySerParameter.class.getName());
		}
		if (!(intermediate instanceof KPABELLW16IntermediateSerParameter)) {
			PairingUtils.NotVerifyCipherParameterInstance(SCHEME_NAME, intermediate,
					KPABELLW16IntermediateSerParameter.class.getName());
		}
		KPABELLW16EncryptionGenerator encryptionGenerator = new KPABELLW16EncryptionGenerator();
		KPABEEncryptionGenerationParameter encryptionGenerationParameter = new KPABEEncryptionGenerationParameter(
				publicKey, attributes, message);
	...
	public PairingKeyEncapsulationSerPair encapsulation(PairingKeySerParameter publicKey,
			PairingCipherSerParameter intermediate, String[] attributes) {
		if (!(publicKey instanceof KPABELLW16PublicKeySerParameter)) {
			PairingUtils.NotVerifyCipherParameterInstance(SCHEME_NAME, publicKey,
					KPABELLW16PublicKeySerParameter.class.getName());
		}
