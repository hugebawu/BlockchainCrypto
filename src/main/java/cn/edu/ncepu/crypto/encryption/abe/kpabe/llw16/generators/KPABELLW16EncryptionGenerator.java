...
public class KPABELLW16EncryptionGenerator extends KPABEHW14EncryptionGenerator {
	...
	public void init(CipherParameters parameter) {
		...
		KPABEEncryptionGenerationParameter resultEncryptionParameter = new KPABEEncryptionGenerationParameter(
				oriEncryptionParameter.getPublicKeyParameter(), mappedStringAttributes,
		...
		if (oriEncryptionParameter.isIntermediateGeneration()) {
			KPABEHW14IntermediateSerParameter intermediateHW14 = new KPABEHW14IntermediateSerParameter(
					...
					this.intermediate.getS(), this.intermediate.getC0(), this.intermediate.getRs(),
					this.intermediate.getXs(), this.intermediate.getC1s(), this.intermediate.getC2s());
	...
	public PairingKeyEncapsulationSerPair generateEncryptionPair() {
		...
		return new PairingKeyEncapsulationSerPair(this.sessionKey.toBytes(),
				new KPABELLW16HeaderSerParameter(publicKeyParameter.getParameters(), chameleonHash, r,
						chameleonHashPublicKey, C01, C02, C0, C1s, C2s, C3s));
	}

	public PairingCipherSerParameter generateCiphertext() {
		computeEncapsulation();
		Element C = this.sessionKey.mul(this.parameter.getMessage()).getImmutable();
		return new KPABELLW16CiphertextSerParameter(publicKeyParameter.getParameters(), chameleonHash, r,
				chameleonHashPublicKey, C01, C02, C, C0, C1s, C2s, C3s);
	}
}
