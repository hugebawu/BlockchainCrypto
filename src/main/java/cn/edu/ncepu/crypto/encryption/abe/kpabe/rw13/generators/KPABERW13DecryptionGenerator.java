...
public class KPABERW13DecryptionGenerator implements PairingDecryptionGenerator, PairingDecapsulationGenerator {
	...
	protected void computeDecapsulation() throws InvalidCipherTextException {
		KPABERW13PublicKeySerParameter publicKeyParameter = (KPABERW13PublicKeySerParameter) this.parameter
				.getPublicKeyParameter();
		KPABERW13SecretKeySerParameter secretKeyParameter = (KPABERW13SecretKeySerParameter) this.parameter
				.getSecretKeyParameter();
		KPABERW13HeaderSerParameter ciphertextParameter = (KPABERW13HeaderSerParameter) this.parameter
		...
		try {
			...
			for (String attribute : omegaElementsMap.keySet()) {
				...
				sessionKey = sessionKey.mul(
						pairing.pairing(C0, K0).mul(pairing.pairing(C1, K1)).mul(pairing.pairing(C2, K2)).powZn(lambda))
						.getImmutable();
			}
	...
	public Element recoverMessage() throws InvalidCipherTextException {
		...
		KPABERW13CiphertextSerParameter ciphertextParameter = (KPABERW13CiphertextSerParameter) this.parameter
				.getCiphertextParameter();
