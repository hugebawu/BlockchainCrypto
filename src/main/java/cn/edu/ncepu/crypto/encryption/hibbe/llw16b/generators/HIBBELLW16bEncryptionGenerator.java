...
public class HIBBELLW16bEncryptionGenerator implements PairingEncryptionGenerator, PairingEncapsulationPairGenerator {
	...
	private void computeEncapsulation() {
		...
		try {
			...
			this.sessionKey = pairing.pairing(publicKeyParameter.getG1(), publicKeyParameter.getG2()).powZn(beta)
					.getImmutable();
	...
	public PairingCipherSerParameter generateCiphertext() {
		...
		try {
			...
			return new HIBBELLW16bCiphertextSerParameter(publicKeyParameter.getParameters(), signPublicKey, signature,
					C0, C1, C2);
	...
	public PairingKeyEncapsulationSerPair generateEncryptionPair() {
		...
		try {
			...
			signer.update(byteArrayC1, 0, byteArrayC1.length);
			byte[] signature = signer.generateSignature();
			return new PairingKeyEncapsulationSerPair(this.sessionKey.toBytes(), new HIBBELLW16bHeaderSerParameter(
					publicKeyParameter.getParameters(), signPublicKey, signature, C0, C1));
		} catch (CryptoException e) {
			e.printStackTrace();
			return null;
		}
