...
public class HIBEBB04DecryptionGenerator implements PairingDecryptionGenerator, PairingDecapsulationGenerator {
	...
	private void computeDecapsulation() throws InvalidCipherTextException {
		HIBEBB04PublicKeySerParameter publicKeyParameters = (HIBEBB04PublicKeySerParameter) this.params
				.getPublicKeyParameter();
		HIBEBB04SecretKeySerParameter secretKeyParameters = (HIBEBB04SecretKeySerParameter) this.params
				.getSecretKeyParameter();
		HIBEBB04HeaderSerParameter ciphertextParameters = (HIBEBB04HeaderSerParameter) this.params
		...
		for (int i = 0; i < Cs.length; i++) {
			if (i < secretKeyLength) {
				...
				d0 = d0.mul(publicKeyParameters.getG1().powZn(elementIdsCT[i]).mul(publicKeyParameters.getHsAt(i)))
						.getImmutable();
	...
	public Element recoverMessage() throws InvalidCipherTextException {
		...
		HIBEBB04CiphertextSerParameter ciphertextParameters = (HIBEBB04CiphertextSerParameter) this.params
				.getCiphertextParameter();
