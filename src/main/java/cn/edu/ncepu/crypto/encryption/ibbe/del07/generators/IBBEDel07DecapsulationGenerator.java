...
import cn.edu.ncepu.crypto.algebra.algorithms.HornerRule;
...
public class IBBEDel07DecapsulationGenerator implements PairingDecapsulationGenerator {
	...
	public byte[] recoverKey() throws InvalidCipherTextException {
		IBBEDel07PublicKeySerParameter publicKeyParameters = (IBBEDel07PublicKeySerParameter) this.params
				.getPublicKeyParameter();
		IBBEDel07SecretKeySerParameter secretKeyParameters = (IBBEDel07SecretKeySerParameter) this.params
				.getSecretKeyParameter();
		IBBEDel07HeaderSerParameter ciphertextParameters = (IBBEDel07HeaderSerParameter) this.params
		...
		if (index >= params.getNumberOfBroadcastReceiver()) {
			throw new InvalidCipherTextException(
					"User identity is not in the broadcast receiver set, cannot decapsulate...");
		}
		...
		for (int i = 0, j = 0; i < params.getNumberOfBroadcastReceiver(); i++) {
			if (i != index) {
				elementsIdsWithoutTarget[j] = PairingUtils
						.MapStringToGroup(pairing, this.params.getIdsAt(i), PairingUtils.PairingGroupType.Zr)
						.getImmutable();
				j++;
			}
		}
		...
		return pairing.pairing(ciphertextParameters.getC1(), temp1)
				.mul(pairing.pairing(secretKeyParameters.getSecretKey(), ciphertextParameters.getC2())).powZn(tempPow)
				.getImmutable().toBytes();
	}
}
