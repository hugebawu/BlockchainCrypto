...
public class IBEGen06bEncryptionGenerator implements PairingEncryptionGenerator, PairingEncapsulationPairGenerator {
	...
	public PairingKeyEncapsulationSerPair generateEncryptionPair() {
		...
		Element u = publicKeyParameter.getG1().powZn(s).mul(publicKeyParameter.getG().powZn(elementId.mul(s).negate()))
				.getImmutable();
		...
		Element sessionKey = pairing.pairing(publicKeyParameter.getG(), publicKeyParameter.getH()).powZn(s)
				.getImmutable();
		...
		Element y = pairing.pairing(publicKeyParameter.getG(), publicKeyParameter.getH2()).powZn(s)
				.mul(pairing.pairing(publicKeyParameter.getG(), publicKeyParameter.getH3()).powZn(s.mulZn(beta)))
				.getImmutable();
	...
	public PairingCipherSerParameter generateCiphertext() {
		...
		Element v = pairing.pairing(publicKeyParameter.getG(), publicKeyParameter.getG()).powZn(s).getImmutable();
		byte[] byteArrayV = v.toBytes();
		Element w = pairing.pairing(publicKeyParameter.getG(), publicKeyParameter.getH()).powZn(s)
				.mul(params.getMessage()).getImmutable();
