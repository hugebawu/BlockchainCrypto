...
public class RELLW16aIntermediateGenerator implements PairingEncryptionGenerator {
	...
	protected void computeEncapsulation() {
		...
		for (int i = 0; i < n; i++) {
			...
			this.C2s[i] = publicKeyParameter.getGb2().powZn(xs[i]).mul(publicKeyParameter.getHb()).powZn(ss[i])
					.getImmutable();
	...
	public PairingCipherSerParameter generateCiphertext() {
		...
		return new RELLW16aIntermediateSerParameter(publicKeyParameter.getParameters(), n, sessionKey, s, C0, ss, xs,
				C1s, C2s);
	}
}