...
public class RELLW16aHeaderSerParameter extends RELSW10aHeaderSerParameter {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 2702642770319727899L;
	private transient Map<String, Element> C3s;
	private final Map<String, byte[]> byteArraysC3s;

	public RELLW16aHeaderSerParameter(PairingParameters pairingParameters, Element C0, Map<String, Element> C1s,
			Map<String, Element> C2s, Map<String, Element> C3s) {
	...
	private void readObject(java.io.ObjectInputStream objectInputStream)
		...
		for (String revokeId : this.byteArraysC3s.keySet()) {
			this.C3s.put(revokeId,
					pairing.getZr().newElementFromBytes(this.byteArraysC3s.get(revokeId)).getImmutable());
		}
	}
}
