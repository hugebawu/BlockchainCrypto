...
public class RELSW10aHeaderSerParameter extends PairingCipherSerParameter {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7000479220635031697L;
	...
	public RELSW10aHeaderSerParameter(PairingParameters pairingParameters, Element C0, Map<String, Element> C1s,
			Map<String, Element> C2s) {
	...
	private void readObject(java.io.ObjectInputStream objectInputStream)
		...
		for (String revokeId : this.byteArraysC1s.keySet()) {
			this.C1s.put(revokeId,
					pairing.getG1().newElementFromBytes(this.byteArraysC1s.get(revokeId)).getImmutable());
			this.C2s.put(revokeId,
					pairing.getG1().newElementFromBytes(this.byteArraysC2s.get(revokeId)).getImmutable());
		}
	}
}
