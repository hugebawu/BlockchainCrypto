...
public class RBACLLW15EngineJUnitTest extends TestCase {
	...
	private void try_patient_valid_decapsulation(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
		...
		try {
			...
			logger.info("Valid patient decapsulation test failed, " + "patient identity    = " + identity + ", "
					+ "encapsulation roles = " + Arrays.toString(roles) + ", " + "encapsulation id.   = " + encIdentity
		...
		try {
			...
			logger.info("Valid patient decapsulation test with intermediate failed, " + "patient identity    = "
					+ identity + ", " + "encapsulation roles = " + Arrays.toString(roles) + ", "
					+ "encapsulation id.   = " + encIdentity + ", " + "encapsulation time  = " + encTime);
			e.printStackTrace();
			System.exit(1);
		}
	}

	private void try_patient_invalid_decapsulation(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
			String identity, String[] roles, String encIdentity, String encTime) {
		try {
			try_patient_decapsulation(publicKey, masterKey, identity, roles, encIdentity, encTime);
			...
			logger.info("Invalid patient decapsulation test failed, " + "patient identity    = " + identity + ", "
					+ "encapsulation roles = " + Arrays.toString(roles) + ", " + "encapsulation id.   = " + encIdentity
					+ ", " + "encapsulation time  = " + encTime);
			e.printStackTrace();
			System.exit(1);
		...
		try {
			...
			logger.info("Invalid patient decapsulation with intermediate test failed, " + "patient identity    = "
					+ identity + ", " + "encapsulation roles = " + Arrays.toString(roles) + ", "
					+ "encapsulation id.   = " + encIdentity + ", " + "encapsulation time  = " + encTime);
			e.printStackTrace();
			System.exit(1);
	...
	private void try_patient_decapsulation(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
			String identity, String[] roles, String encIdentity, String encTime)
		...
		// Decryption
		byte[] anSessionKey = engine.EHRDecPWithAudit(publicKey, encIdentity, roles, encTime, ciphertext,
				accessCredentialP);
		Assert.assertArrayEquals(sessionKey, anSessionKey);
	}

	private void try_patient_decapsulation_with_intermediate(PairingKeySerParameter publicKey,
			PairingKeySerParameter masterKey, String identity, String[] roles, String encIdentity, String encTime)
			throws InvalidCipherTextException, IOException, ClassNotFoundException {
		...
		// encapsulation and serialization without intermediate ciphertext
		PairingKeyEncapsulationSerPair encapsulationSerPair = engine.EHREnc(publicKey, intermediateParameter,
	...
	private void try_medical_valid_decapsulation(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
		...
		try {
			...
			logger.info("Valid medical staff decapsulation test failed, " + "medical staff roles = "
					+ Arrays.toString(medicalRoles) + ", " + "medical staff time  = " + medicalTime + ", "
					+ "encapsulation roles = " + Arrays.toString(roles) + ", " + "encapsulation id.   = " + encIdentity
					+ ", " + "encapsulation time  = " + encTime);
			e.printStackTrace();
			System.exit(1);
		...
		try {
			try_medical_decapsulation_with_intermediate(publicKey, masterKey, medicalRoles, medicalTime, roles,
					encIdentity, encTime);
		} catch (Exception e) {
			logger.info("Valid medical staff decapsulation with intermediate test failed, " + "medical staff roles = "
					+ Arrays.toString(medicalRoles) + ", " + "medical staff time  = " + medicalTime + ", "
					+ "encapsulation roles = " + Arrays.toString(roles) + ", " + "encapsulation id.   = " + encIdentity
					+ ", " + "encapsulation time  = " + encTime);
			e.printStackTrace();
	...
	private void try_medical_invalid_decapsulation(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
			String[] medicalRoles, String medicalTime, String[] roles, String encIdentity, String encTime) {
		try {
			try_medical_decapsulation(publicKey, masterKey, medicalRoles, medicalTime, roles, encIdentity, encTime);
		} catch (InvalidCipherTextException e) {
			...
			logger.info("Invalid medical staff decapsulation test failed, " + "medical staff roles = "
					+ Arrays.toString(medicalRoles) + ", " + "medical staff time  = " + medicalTime + ", "
					+ "encapsulation roles = " + Arrays.toString(roles) + ", " + "encapsulation id.   = " + encIdentity
					+ ", " + "encapsulation time  = " + encTime);
			e.printStackTrace();
		...
		try {
			...
			logger.info("Invalid medical staff decapsulation with intermediate test failed, " + "medical staff roles = "
					+ Arrays.toString(medicalRoles) + ", " + "medical staff time  = " + medicalTime + ", "
					+ "encapsulation roles = " + Arrays.toString(roles) + ", " + "encapsulation id.   = " + encIdentity
					+ ", " + "encapsulation time  = " + encTime);
			e.printStackTrace();
	...
	private void try_medical_decapsulation(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
			String[] medicalRoles, String medicalTime, String[] roles, String encIdentity, String encTime)
			throws InvalidCipherTextException, IOException, ClassNotFoundException {
		// KeyGen and serialization
		...
		// Decryption
		byte[] anSessionKey = engine.EHRDecMWithAudit(publicKey, encIdentity, roles, encTime, ciphertext,
				accessCredentialM);
		Assert.assertArrayEquals(sessionKey, anSessionKey);
	}

	private void try_medical_decapsulation_with_intermediate(PairingKeySerParameter publicKey,
			PairingKeySerParameter masterKey, String[] medicalRoles, String medicalTime, String[] roles,
	...
	private void try_medical_valid_delegation(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
			String[] medicalRoles, String medicalTime, int index, String role, String[] roles, String encIdentity,
			String encTime) {
		try {
			try_medical_delegation(publicKey, masterKey, medicalRoles, medicalTime, index, role, roles, encIdentity,
					encTime);
		} catch (Exception e) {
			logger.info("Valid medical staff decapsulation test failed, " + "medical staff roles = "
					+ Arrays.toString(medicalRoles) + ", " + "medical staff time  = " + medicalTime + ", "
					+ "delegation index    = " + index + ", " + "delegation role     = " + role + ", "
					+ "encapsulation id.   = " + encIdentity + ", " + "encapsulation time  = " + encTime);
			e.printStackTrace();
			System.exit(1);
		}
		try {
			try_medical_delegation_with_intermediate(publicKey, masterKey, medicalRoles, medicalTime, index, role,
					roles, encIdentity, encTime);
		} catch (Exception e) {
			logger.info("Valid medical staff decapsulation with intermediate test failed, " + "medical staff roles = "
					+ Arrays.toString(medicalRoles) + ", " + "medical staff time  = " + medicalTime + ", "
					+ "delegation index    = " + index + ", " + "delegation role     = " + role + ", "
	...
	private void try_medical_invalid_delegation(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
			String[] medicalRoles, String medicalTime, int index, String[] roles, String encIdentity, String encTime) {
		try {
			try_medical_delegation(publicKey, masterKey, medicalRoles, medicalTime, index, "Role_1", roles, encIdentity,
					encTime);
		} catch (InvalidCipherTextException e) {
			// correct if getting there, nothing to do.
		} catch (Exception e) {
			logger.info("Invalid medical staff decapsulation test failed, " + "medical staff roles = "
					+ Arrays.toString(medicalRoles) + ", " + "medical staff time  = " + medicalTime + ", "
					+ "delegation index    = " + index + ", " + "delegation role     = " + "Role_1" + ", "
					+ "encapsulation id.   = " + encIdentity + ", " + "encapsulation time  = " + encTime);
			e.printStackTrace();
			System.exit(1);
		}
		try {
			try_medical_delegation_with_intermediate(publicKey, masterKey, medicalRoles, medicalTime, index, "Role_1",
					roles, encIdentity, encTime);
		} catch (InvalidCipherTextException e) {
			// correct if getting there, nothing to do.
		} catch (Exception e) {
	...
	private void try_medical_delegation(PairingKeySerParameter publicKey, PairingKeySerParameter masterKey,
			String[] medicalRoles, String medicalTime, int index, String role, String[] roles, String encIdentity,
			String encTime) throws InvalidCipherTextException, IOException, ClassNotFoundException {
		...
		// Decryption
		byte[] anSessionKey = engine.EHRDecMWithAudit(publicKey, encIdentity, roles, encTime, ciphertext,
				accessCredentialDeleM);
		Assert.assertArrayEquals(sessionKey, anSessionKey);
	}

	private void try_medical_delegation_with_intermediate(PairingKeySerParameter publicKey,
			PairingKeySerParameter masterKey, String[] medicalRoles, String medicalTime, int index, String role,
			String[] roles, String encIdentity, String encTime)
			throws InvalidCipherTextException, IOException, ClassNotFoundException {
		// generate intermediate
		PairingCipherSerParameter intermediateParameter = engine.IntermediateGen(publicKey);
		byte[] byteArrayIntermediateParameter = CommonUtils.SerObject(intermediateParameter);
	...
	public void testRBACLLW15Engine() {
		...
		this.runAllTests(PairingFactory.getPairingParameters(PairingUtils.TEST_PAIRING_PARAMETERS_PATH_a_80_256));
	}
}
