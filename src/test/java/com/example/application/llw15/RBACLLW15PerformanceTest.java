...
public class RBACLLW15PerformanceTest {
	...
	// the maximal depth of roles is chosen according to the full version of our
	// paper.
...
//        }
	...
	private void test_one_round() {
		...
		timer.start(0);
		accessCredentialPatient = engine.ACGenP(publicKey, masterKey, intermediateParametersAccessCredentialPaitent,
				patientId);
		temperTime = timer.stop(0);
		out.print("\t" + temperTime);
		...
		for (int i = 0; i < maximal_depth; i++) {
			...
			timer.start(i);
			accessCredentialMedicalStaff[i] = engine.ACGenM(publicKey, masterKey,
					accessCredentialMedicalStaffIntermediateParameters[i], medicalStaffRoleVectors[i], timeT);
			temperTime = timer.stop(i);
			out.print("\t" + temperTime);
		...
		for (int i = 0; i < maximal_roles; i++) {
			timer.start(i);
			PairingKeyEncapsulationSerPair encapsulationPair = engine.EHREnc(publicKey, patientId,
					encapsulationRoleVectorSets[i], timeT);
		...
		for (int i = 0; i < maximal_roles; i++) {
			...
			timer.start(i);
			PairingKeyEncapsulationSerPair encapsulationPair = engine.EHREnc(publicKey,
					encapsulationsIntermedicateParameters[i], patientId, encapsulationRoleVectorSets[i], timeT);
			encapsulations[i] = encapsulationPair.getHeader();
			temperTime = timer.stop(i);
			out.print("\t" + temperTime);
		...
		for (int i = 0; i < maximal_roles; i++) {
			try {
				timer.start(i);
				engine.EHRDecP(publicKey, patientId, encapsulationRoleVectorSets[i], timeT, encapsulations[i],
						accessCredentialPatient);
				temperTime = timer.stop(i);
				out.print("\t" + temperTime);
		...
		for (int i = 0; i < maximal_depth; i++) {
			...
			for (int j = i; j < maximal_roles; j++) {
				try {
					timer.start(i);
					engine.EHRDecM(publicKey, patientId, encapsulationRoleVectorSets[j], timeT, encapsulations[j],
							accessCredentialMedicalStaff[i]);
					temperTime = timer.stop(i);
					out.print("\t" + temperTime);
