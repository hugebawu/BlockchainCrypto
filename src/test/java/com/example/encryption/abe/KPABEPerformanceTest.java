...
public class KPABEPerformanceTest extends TestCase {
	private static final Logger logger = LoggerFactory.getLogger(KPABEPerformanceTest.class);
...
//            logger.info(this.accessPolicies[i]);
//        }
	}
		...
		try {
			...
			if (this.engine instanceof OOKPABEEngine) {
				...
				for (int i = 0; i < maximal_attributes; i++) {
					...
					timer.start(i);
					onlineHeaders[i] = ooEngine.encapsulation(publicKey, intermediates[i], attributeSets[i])
							.getHeader();
					temperTime = timer.stop(i);
					out.print("\t" + temperTime);
